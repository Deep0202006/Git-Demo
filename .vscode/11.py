...
class UserSession(db.Model):
    ...
    login_time = db.Column(db.String(80), nullable=False)
    logout_time = db.Column(db.String(80), nullable=True)
...
def login():
    ...
    if session:
        ...
        session.logout_time = None  # Reset logout time on new login
        session.total_time = 0  # Reset total time on new login
    else:
        session = UserSession(user_id=user_id, login_time=login_time)
        db.session.add(session)
    
    db.session.commit()
    return jsonify({"message": "Login time recorded", "data": session}), 200
...
def logout():
    ...
    db.session.commit()
    return jsonify({"message": "Logout time recorded", "data": session}), 200
...
def view_user_sessions(self):
    ...
    if response.status_code == 200:
        ...
        # Create a new Toplevel window to display user sessions
        session_window = tk.Toplevel(self.root)
        session_window.title("User  Sessions")
        session_window.geometry("400x300")
        
        # Create a Text widget to display the sessions
        session_text = tk.Text(session_window, height=15, bg='#34495e', fg='white')
        session_text.pack(pady=10, padx=10, fill=tk.BOTH, expand=True)

        # Clear previous text
        session_text.delete(1.0, tk.END)  
        
        if not sessions:
            session_text.insert(tk.END, "No user sessions found.\n")
        else:
            for session in sessions:
                session_text.insert(tk.END, f"User  ID: {session['user_id']}\n")
                session_text.insert(tk.END, f"Login Time: {session['login_time']}\n")
                session_text.insert(tk.END, f"Logout Time: {session['logout_time']}\n")
                session_text.insert(tk.END, f"Total Time Spent: {session['total_time']} seconds\n\n")
    else:
