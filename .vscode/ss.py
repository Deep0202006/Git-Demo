    ...
    def view_user_sessions(self):
       # Check if api_data_text is initialized
        ...
        if response.status_code == 200:
            ...
            for session in sessions:
                self.api_data_text.insert(tk.END, f"User  ID: {session['user_id']}\n")
                self.api_data_text.insert(tk.END, f"Login Time: {session['login_time']}\n")
                self.api_data_text.insert(tk.END, f"Logout Time: {session['logout_time']}\n")
                self.api_data_text.insert(tk.END, f"Total Time Spent: {session['total_time']} seconds\n\n")
        else:
